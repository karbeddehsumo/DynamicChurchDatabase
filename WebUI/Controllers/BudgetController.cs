   ...
   //  [RoleAuthentication(Roles = "WebMaster,Admin,Pastor,FinanceLead")]
    public class BudgetController : Controller
    {
        ...
        public ActionResult IncomeBudgetList(int BudgetYear)
        {
            ...
            {
                IncomeRepository.InitializeVariable();
                i.subcategory = SubCategoryRepository.GetBySubCategoryID(i.SubCategoryID);
                IEnumerable<income> IncomeList = IncomeRepository.GetIncomeChildren(i.SubCategoryID, BeginDate, EndDate);
                foreach (var m in IncomeList)
                {
                    if (m.CashAmount == null) { cash = 0; } else { cash = (decimal)m.CashAmount; };
                    if (m.CheckAmount == null) { check = 0; } else { check = (decimal)m.CheckAmount; };
                    if (m.CoinAmount == null) { coin = 0; } else { coin = (decimal)m.CoinAmount; };
        ...
        public ActionResult ExpenseBudgetList(int BudgetYear)
        {
            ...
            foreach (var i in BudgetList)
            {
                ExpenseRepository.InitializeVariable();
                i.subcategory = SubCategoryRepository.GetBySubCategoryID(i.SubCategoryID);
                i.TotalActualAmount = ExpenseRepository.GetExpenseChildren(i.SubCategoryID, BeginDate, EndDate).Sum(e => e.Amount);
                i.Ratio = i.TotalActualAmount / i.Amount;
            }
            decimal BudgetSum = BudgetList.Sum(e => e.TotalActualAmount);
        ...
        public ActionResult BudgetCategoryDetails(int SubCategoryID, int categoryID)
        {
            ...
            if (ViewBag.CategoryType == "Income")
            {
                ...
                //Get totals for budget subcategories
                ViewBag.BeginDate = BeginDate;
                ViewBag.EndDate = EndDate;
                List<SubcategoryStore> BudgetSubcategoryList = new List<SubcategoryStore>();

                List<int> subcategoryList = IncomeList2.Select(e => e.subCategoryID).Distinct().ToList();
                foreach (var i in subcategoryList)
                {
                    SubcategoryStore item = new SubcategoryStore();
                    IncomeRepository.InitializeVariable();
                    item.subcategoryID = i;
                    item.Amount = IncomeRepository.GetTotalIncomeByCategory(i, BeginDate, EndDate);
                    //item.Ratio = item.Amount / i.Amount;
                    item.Name = SubCategoryRepository.GetBySubCategoryID(i).SubCategoryName;
                    BudgetSubcategoryList.Add(item);
                }
                ViewBag.BudgetSubcategoryList = BudgetSubcategoryList;      
            }
            else
            {
                ...
                List<int> subcategoryList = ExpenseList2.Select(e => e.subCategoryID).Distinct().ToList();
                foreach (var i in subcategoryList)
                {
                    SubcategoryStore item = new SubcategoryStore();
                    ExpenseRepository.InitializeVariable();
                    item.subcategoryID = i;
                    item.Amount = ExpenseRepository.GetExpenseChildren(i, BeginDate, EndDate).Sum(e => e.Amount);
                    //item.Ratio = item.Amount / i.Amount;
                    item.Name = SubCategoryRepository.GetBySubCategoryID(i).SubCategoryName;
                    BudgetSubcategoryList.Add(item);
                }

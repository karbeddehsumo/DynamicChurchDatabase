  ...
  //  [RoleAuthentication(Roles = "WebMaster,Admin,Pastor,FinanceLead")]
    public class PledgeController : Controller
    {
        ...
        public ActionResult ListMember(int memberID)
        {
            ...
            IEnumerable<pledge> pledge = PledgeRepository.GetPledgeByMember(memberID, bDate.Year);
            if (pledge != null)
            {
                ViewBag.RecordCount = pledge.Count();
                foreach (var i in pledge.Where(e => e.PledgeYear == year).ToList())
                {

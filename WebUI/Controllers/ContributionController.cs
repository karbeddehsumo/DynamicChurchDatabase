...
namespace WebUI.Controllers
{
    ...
    public class ContributionController : Controller
    {
      ...
      //  public ActionResult Create(DateTime dDate , int memberID = 0)
        public ActionResult Create( DateTime dDate, int memberID = 0)
        {
            var repeatDate = TempData["ContributionDate"];
            if (repeatDate != null)
            {
                ViewBag.DefaultDate = repeatDate;
                ViewBag.MemberID = TempData["memberID"];
            }
            else
            {
        ...
        public ActionResult Create(contribution contribution)
        {
            ...
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    TempData["memberID"] = contribution.memberID;
                    TempData["ContributionDate"] = contribution.ContributionDate;
                    GetData();
        ...
        public void GetData()
        {
            ...
            Dictionary<string, string> TaxReportYear = new Dictionary<string, string>();
            int byear = DateTime.Now.Year;
            for (int i = byear - 1; i <= byear + 1; i += 1)
            {
                TaxReportYear.Add(i.ToString(), i.ToString());
            }
            ViewBag.TaxReportYear = new SelectList(TaxReportYear, "Key", "Value", id);
        }
        ...
        [RoleAuthentication(Roles = "WebMaster,Admin,Pastor,FinanceLead,FinanceStaff,Admin2")]
        public void TaxReport(int memberID, int Year)
        {
            string FileName = string.Format("{0}", "ContributionTaxFile.pdf");
            // var path = Path.Combine(Server.MapPath("~/public_html/ClientFiles"), FileName);
            var path = Path.Combine(Server.MapPath("~/App_Data/ClientFiles"), FileName);
